...
***
#### Definición:
La `r params$nombreindicador` es la relación del total de muertes de personas menores de 18 años (NNA) por suicidios por cada 100.000 habitantes.
...
$$ TMSnna^t= \frac{\sum_i^n{x_{i}^t}}{NNA^t} * 100.000 $$


- $TMSnna^t$    : `r params$nombreindicador` en el periodo/año **t**.

- $x_{i}^t$  : Defunción del NNA **i** en el periodo **t** por a causa de suicidio

- $i$        : Defunción por suicidio del NNA  **i**.
...
La agrupación de las causas de muertes por suicidio se realiza en función de la Clasificación Estadística Internacional de Enfermedades y Problemas Relacionados con la Salud (CIE):
...
#f. Identificacion de Muertes violentas por suicidio
#crea la variable 'suicidio'


fc_suicidios <- function(data, var) {
  
  var <- enquo(var)
  
  data %>% 
    
    mutate(suicidio= ifelse(str_detect(string = !!var, 
                                              pattern = '^SUICI'), 
                                   yes = T, 
                                   no = F)
    )
...
#f2. funcion de conteo de casos del indicador
  #varindicad: variables creadas con las funciones fc_suicidios y fc_nna
